This QGIS plugin facilitates the **data preparation for the rockfall simulation model Rockyfor3D**. Based on a digital elevation model (DEM) and a polygon or line input (terrain / forest / rockfall nets), the plugin **rasterizes** the attributes from the vector layer, ensuring **alignment** of all output rasters with the DEM grid, and the **correct output format** of all data. 
